...
export const getQuestionsHandler = async (ws: WebSocket, roomId: string, players: PlayerState[]) => {
  ...
  try {
    ...
    if (users.length <= 0) {
      return sendResponse(ws, QUESTION_GENERATOR_ERROR_RESPONSE, {
    ...
    try {
      ...
      if (tokenStateJson) {
        ...
        if (tokenState.roomId !== roomId) {
          sendResponse(ws, QUESTION_GENERATOR_ERROR_RESPONSE, {
            errorCode: QUESTION_GENERATOR_CODES.PLAYER_NOT_IN_ROOM,
          });
        }

        if (!tokenState.authToken) {
          sendResponse(ws, QUESTION_GENERATOR_ERROR_RESPONSE, {
            errorCode: QUESTION_GENERATOR_CODES.AUTH_TOKEN_NOT_FOUND,
          });
        }
      ...
      sendResponse(ws, QUESTION_GENERATOR_ERROR_RESPONSE, {
        errorCode: QUESTION_GENERATOR_CODES.GET_USERS_FROM_PLAYER_STATE_FAILED,
      });
    }
  }
  return users;
